...
psp_display_screen_settings_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_SET_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SET_COLOR) {
      if (dve_color) strcpy(buffer,"blue");
    ...
    if (menu_id == MENU_SET_RENDER) {
      ...
      if (dve_render_mode == DVE_RENDER_MAX      ) strcpy(buffer, "max");
      else 
      if (dve_render_mode == DVE_RENDER_X15      ) strcpy(buffer, "x15");
      else 
      if (dve_render_mode == DVE_RENDER_FIT      ) strcpy(buffer, "fit");
...
{
  if (step > 0) {
    if (dve_speed_limiter < 40) dve_speed_limiter++;
    else                        dve_speed_limiter = 0;
  } else {
    if (dve_speed_limiter > 0) dve_speed_limiter--;
    else                       dve_speed_limiter = 40;
  }
}
...
psp_settings_menu_init(void)
{
  ...
  psp_display_lr       = DVE.psp_display_lr;
}
...
psp_settings_menu_validate(void)
{
  ...
  DVE.psp_display_lr        = psp_display_lr;
...
# endif
        ...
        (new_pad & PSP_CTRL_LEFT  )) 
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        break;              
        case MENU_SET_COLOR       : dve_color = ! dve_color;
        break;              
