...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 20, "Game: %s", DVE.dve_save_name);
...
psp_display_screen_menu(void)
{
  ...
  for (slot_id = 0; slot_id < DVE_MAX_SAVE_STATE; slot_id++) {
    ...
    if (DVE.dve_save_state[slot_id].used) {
      ScePspDateTime* my_date = &DVE.dve_save_state[slot_id].date;
# ifndef LINUX_MODE
...
# endif
  ...
  if (DVE.dve_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(320, 40, DVE.dve_save_state[cur_slot].surface);
  } else {
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print(140, 160, "Reset DVE !", PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
  dve_emulator_reset();
  dve_reset_save_name();
